...
describe('Pricing Engine - Golden Fixtures', () => {
  ...
  test('Fixture 7: Short-term premium structure', () => {
    ...
    expect(term9!.price).toBeGreaterThanOrEqual(baseline * 1.0);
...
describe('Pricing Engine - Invariants', () => {
  test('Invariant: Floors never violated', () => {
    ...
    for (const scenario of scenarios) {
      ...
      const minFloor = Math.max(500, scenario.unit.currentRent * 0.9);
  ...
  test('Invariant: Directional caps respected', () => {
    ...
    const result = priceUnit(lowOccScenario.unit, lowOccScenario.config, lowOccScenario.context);
  ...
  test('Invariant: Distance-to-target monotonicity', () => {
    ...
    scenario2.context.floorplanTrends['A1'].trending = 0.8;
    ...
    const move1Pct =
      (result1.baselineRent - scenario1.unit.currentRent) / scenario1.unit.currentRent;
    const move2Pct =
      (result2.baselineRent - scenario2.unit.currentRent) / scenario2.unit.currentRent;
