...
import type { PricingDataProvider, BoxScoreData, LeadsAppsData } from './PricingDataProvider';
...
export class RealDataProvider implements PricingDataProvider {
  ...
  private checkDataAvailability(): boolean {
    ...
    const hasMappedRows =
      Array.isArray((window as any).mappedRows) && (window as any).mappedRows.length > 0;
    const hasPropertySetup =
      (window as any).propertySetup && Array.isArray((window as any).propertySetup.floorplans);
  ...
  async getBoxScore(): Promise<BoxScoreData> {
    ...
    for (const fp of floorplans) {
      ...
      const occupied = fpUnits.filter(
        u => !u.status.includes('vacant') || u.status.includes('notice')
      ).length;
    ...
    const totalOccupied = units.filter(
      u => !u.status.includes('vacant') || u.status.includes('notice')
    ).length;
      ...
      for (const [unitId, value] of Object.entries(data)) {
        if (typeof value === 'object' && value !== null) {
          ...
          baselines[unitId] = {
            ...
            priorApprovedDate:
              baseline.date || baseline.priorApprovedDate || new Date().toISOString(),
   ...
   */
  async getFloorplanSetup(): Promise<
    Array<{
      code: string;
      name: string;
      bedrooms: number;
