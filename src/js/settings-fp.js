(function () {
  ...
  function isRowValid(r) {
    ...
    const l = Number(r.band_low_pct),
      h = Number(r.band_high_pct);
    ...
    if (
      r.price_floor_dollars != null &&
      String(r.price_floor_dollars) !== '' &&
      Number(r.price_floor_dollars) < 0
    )
      return false;
    if (
      r.price_ceiling_dollars != null &&
      String(r.price_ceiling_dollars) !== '' &&
      Number(r.price_ceiling_dollars) < 0
    )
      return false;
    return true;
  }
  function isTableValid() {
    ...
    for (const r of rows) {
      if (String(r.code || '') && dups.get(normalizeCode(r.code)) > 1) return false;
    }
  ...
  function loadFPsLocal() {
    try {
      ...
      if (Array.isArray(parsed)) {
        window.propertySetup.floorplans = parsed.map(r => {
          const { stop_down_buffer_dollars, ...rest } = r || {};
          return rest;
        });
      }
  ...
  function renderFPTable() {
    ...
    function _bedOf(code, name) {
      const c = String(code || '');
      const s = (String(code || '') + ' ' + String(name || '')).toLowerCase();
      if (
        /\b(s0|studio|0x1)\b/i.test(c) ||
        /\b(studio|0x1)\b/.test(String(name || '').toLowerCase())
      )
        return 0;
    ...
    tbody.innerHTML = rows
      .map((r, idx) => {
        ...
        const badges = [];
        if (code && dups.get(code) > 1)
          badges.push('<span class="badge danger">Duplicate code</span>');
    ...
    tbody.querySelectorAll('[data-action="del"]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.preventDefault();
        deleteFPRow(Number(btn.dataset.i));
      });
    });
    try {
  ...
  function setFP(idx, key, value) {
    ...
    if (
      key === 'units' ||
      key === 'reference_ask' ||
      key === 'gap_to_next_tier_dollars' ||
      key === 'price_floor_dollars' ||
      key === 'price_ceiling_dollars'
    ) {
      if (String(value) === '') {
        rows[idx][key] = '';
        renderFPTable();
    ...
    if (key === 'band_low_pct' || key === 'band_high_pct')
      value = clamp(Math.round(Number(value || 0) * 2) / 2, 70, 100);
    rows[idx][key] = value;
  ...
  function codeFromLabel(label) {
    ...
    const sanitized = tok || String(label || '').replace(/[^A-Za-z0-9]/g, '');
    return (sanitized || 'FP').slice(0, 12);
  }
  function prefillFromRentRoll() {
    ...
    for (const r of mr) {
      ...
      if (!key) continue;
      const cur = freq.get(key) || { label: lbl, units: 0 };
      cur.units++;
      if (!cur.label) cur.label = lbl;
      freq.set(key, cur);
    }
    const out = [];
    for (const { label, units } of freq.values()) {
      ...
      out.push({
        ...
        name: label,
        units,
        band_low_pct: small ? 92 : 93,
        band_high_pct: small ? 97 : 96,
        gap_to_next_tier_dollars: 0,
        reference_ask: 0,
        price_floor_dollars: '',
        price_ceiling_dollars: '',
  ...
  function mergeFromRentRoll() {
    ...
    for (const { label, units } of freq.values()) {
      ...
      if (t) {
        rows[t.i].units = units;
      } else {
        let c = codeFromLabel(label);
        let base = c.toLowerCase();
        let n = 1;
        while (usedCodes.has(base)) {
          n++;
          c = (c + n).slice(0, 12);
          base = c.toLowerCase();
        }
  ...
  function initFPSettings() {
    const add = document.getElementById('addFPBtn');
    if (add) add.addEventListener('click', addFPRow);
    const pf = document.getElementById('prefillFromRR');
    if (pf) {
      if (Array.isArray(window.mappedRows) && window.mappedRows.length) {
        pf.style.display = 'inline';
        pf.addEventListener('click', e => {
          e.preventDefault();
          const rows = (window.propertySetup && window.propertySetup.floorplans) || [];
          if (!rows.length) prefillFromRentRoll();
          else mergeFromRentRoll();
    ...
    if (clear)
      clear.addEventListener('click', e => {
        e.preventDefault();
        if (confirm('Clear locally saved data?')) {
          try {
          ...
          try {
            ...
            localStorage.removeItem(`rm:fpmap:${pid}`);
            localStorage.removeItem(`rm:fpmap:candidates:${pid}`);
          } catch (err) {}
  ...
  if (document.readyState === 'loading')
    document.addEventListener('DOMContentLoaded', initFPSettings);
  else initFPSettings();
})();
