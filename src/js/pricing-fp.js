...
(function(){
  ...
  function firstToken(label){ 
    const raw=String(label||'').replace(/[×✕✖]/g,'x').trim().replace(/\s+/g,' '); 
  ...
  function renderNewLease(cfg,norm,tState){
    const wrap=document.getElementById("nlTables"); wrap.innerHTML="";
    ...
    for(const [fp,list] of byFP.entries()){
      ...
      wrap.insertAdjacentHTML("beforeend",`
        <div style="border:1px solid #0b2035;border-radius:10px;padding:10px;margin:8px 0">
          ...
          ${explain([
            (tState.tFP[code] ?? tState.tFP[fp])>document.getElementById("comfortHigh").value/100? "Trend above comfort":"Trend below/within comfort",
