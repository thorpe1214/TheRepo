(function(){
  ...
  function loadFPMapLegacy(){ try{ const raw=localStorage.getItem(getFPMapKey()); return raw? JSON.parse(raw) : {}; }catch(e){ return {}; } }
  function saveFPMapLegacy(map){ try{ localStorage.setItem(getFPMapKey(), JSON.stringify(map||{})); }catch(e){} }
  function clearFPMapLegacy(){ try{ localStorage.removeItem(getFPMapKey()); }catch(e){} }
  ...
  function normalizeRentRoll(){
    ...
    const map = loadFPMapLegacy(); const codes = getAllCodes();
