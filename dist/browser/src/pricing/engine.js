 ...
 */
function computeTrendMove(floorplanCode, context, config) {
    const reasons = [];
    ...
    if (priceResponse === 'fast')
        maxMove = 0.08; // Aggressive: ±8%
    else if (priceResponse === 'gentle')
        maxMove = 0.03; // Conservative: ±3%
 ...
 * - reasons: Explanations
 */
function computeConversionNudge(floorplanCode, isInsideBand, context) {
    const reasons = [];
 ...
 * - reasons: Explanations
 */
function determineBaseline(unit, config, context) {
    const reasons = [];
 ...
 * - 10+ mo: 0%
 */
function shortTermPremium(term) {
    if (term >= 10)
        return 0;
 ...
 */
function seasonalityUplift(term, referenceTerm, endMonth, // 0-11
config) {
    if (!config.seasonalityEnabled)
        return 0;
    if (term <= referenceTerm)
        return 0; // Only over-cap terms
 ...
 */
function applyDirectionalCap(proposedRent, baseline, config) {
 ...
 */
function applyFloor(proposedRent, currentRent, config) {
 ...
 */
function applyTierGap(proposedRent, floorplanCode, lowerTierMaxRent, config) {
 ...
 */
function vacancyAgeDiscount(vacantDays, config) {
    ...
    const discountPct = Math.min(daysOver * config.vacancyAgePricing.discountPerDay, config.vacancyAgePricing.maxDiscount);
    return {
 ...
 */
export function priceUnit(unit, config, context, lowerTierMaxRent = null) {
    const reasons = [];
    const flags = {
        trendUp: false,
        trendDown: false,
    ...
    for (const term of config.availableTerms) {
        ...
        const parts = [];
        if (shortPct !== 0)
            parts.push(`Short: +${(shortPct * 100).toFixed(1)}%`);
        if (seasPct !== 0)
            parts.push(`Seasonal: +${(seasPct * 100).toFixed(1)}%`);
        if (vacDiscount.discount !== 0)
            parts.push(`Vacancy: -${(vacDiscount.discount * 100).toFixed(1)}%`);
 ...
 */
export function priceFloorplan(units, config, context, lowerTierMaxRent = null) {
 ...
 */
export function priceAllUnits(state) {
    ...
    for (let i = 0; i < fpCodes.length; i++) {
        ...
        floorplanPricing[code] = {
            ...
            flags: results.length > 0 ? results[0].flags : {},
        };
    }
    return {
        unitPricing,
